...
# New data structure for index [<keyword>, [[url1, count1], [url2, count2]]]
index = ["", [["", 0]]]


def record_user_click(index, keyword, url):
    """
    This function is designed to simulate the user's click on web pages
    :param index: Searching Index of the search engine
    :param keyword: keyword input by users
    :param url: the url of the keyword
    :return: None
    """
    for ele in index:
        if ele[0] == keyword:   # Find the keyword
            for element in ele[1]:
                if element[0] == url:  # Find the url
                    element[1] += 1    # Add one to the click count


def add_to_index(index, keyword, url):
    for entry in index:
        if entry[0] == keyword:
            # The for loop is designed to guarantee that there is only one url in the index for same keyword
            for ele in entry[1]:
                if ele[0] == url:
                    return
